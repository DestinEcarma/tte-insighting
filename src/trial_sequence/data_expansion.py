...
def expand(
    ...
    temp_data.loc[
        (sw_data["eligible"] == 1)
        & sw_data["treatment"].notna()
        & (minperiod <= sw_data["period"])
        & (sw_data["period"] <= maxperiod),
        "expand",
    ] = 1
    ...
    temp_data.loc[sw_data["eligible"] == 1, "init"] = sw_data.loc[
        sw_data["eligible"] == 1, "treatment"
    ]
    ...
    switch_data = pd.DataFrame(
        {
            ...
            "switch_new": (
                sw_data.iloc[expand_index]["switch"] if use_censor else 0
            ),
    ...
    switch_data["trial_period"] = np.concatenate(
        [np.arange(p + 1) for p in sw_data["period"]]
    )
    switch_data["index"] = np.arange(1, len(switch_data) + 1)

    switch_data = switch_data.merge(
        temp_data,
        how="left",
        left_on=["id", "trial_period"],
        right_on=["id", "period"],
    )
    switch_data.sort_values(by="index", inplace=True)
    switch_data["followup_time"] = (
        switch_data["period_new"] - switch_data["trial_period"]
    )

    if "dose" in keeplist:
        switch_data["dose"] = (
            switch_data["cumA_new"]
            - switch_data["dosesum"]
            + switch_data["treat"]
        )
    ...
    for (_, _), group in switch_data[switch_data["expand"] == 1].groupby(
        ["id", "trial_period"]
    ):
        switch_data.loc[group.index, "expand"] = expand_until_switch(
            group["switch_new"]
        )

    switch_data["weight"] = (
        switch_data["weight0"] / switch_data["wtprod"]
    ).dropna(axis=0)
    switch_data["case"] = 0

    if not use_censor:
        switch_data.loc[
            (switch_data["time_of_event"] == switch_data["period_new"])
            & (switch_data["outcome_new"] == 1),
            "case",
        ] = 1
    else:
...
def expand_until_switch(s):
    ...
    if first_switch != -1:
        return np.concatenate(
            [np.ones(first_switch), np.zeros(len(s) - first_switch)]
        )
